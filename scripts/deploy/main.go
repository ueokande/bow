...
func deploy(tag, path string) error {
	...
	release, _, err := client.Repositories.GetReleaseByTag(ctx, "ueokande", "bow", tag)
	if isNotFoundError(err) {
		release, _, err = client.Repositories.CreateRelease(ctx, "ueokande", "bow", &github.RepositoryRelease{
			TagName: strptr(tag),
	...
	asset, _, err := client.Repositories.UploadReleaseAsset(ctx, "ueokande", "bow", release.GetID(), opt, f)
	if err != nil {
